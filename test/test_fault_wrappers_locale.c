...
static void test_p101_setlocale(struct p101_env *env, struct p101_error *err)
{
    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            char *native_result = p101_setlocale(native_env, native_err, LC_ALL, "C");
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                ...
                {
                    fprintf(stderr, "native smoke produced an undeclared platform failure: p101_setlocale: %s\n", p101_error_get_message(native_err));
                    native_passed = false;
                }
